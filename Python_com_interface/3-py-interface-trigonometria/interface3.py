...
class CalculadoraTrigonometrica:
    ...
    def carregar_icone(self):
        try:
            ...
            self.janela.iconphoto(True, self.icone_tk)
    ...
    def criar_imagem_topo(self):
        try:
            ...
            tk.Label(
                self.janela,
                text="Imagem 'seno2.png' não encontrada",
                bg="#f0f0f0"
            ).pack(pady=20)
    ...
    # Cria os botões de calcular e limpar
    ...
    def criar_resultados(self):
        self.frame_resultados = tk.Frame(self.janela, bg="#f0f0f0")
        self.frame_resultados.pack(pady=10)

        self.resultado_seno = self.criar_linha_resultado(
            self.frame_resultados, 0, "Seno:"
        )
        self.resultado_cosseno = self.criar_linha_resultado(
            self.frame_resultados, 1, "Cosseno:"
        )
        self.resultado_tangente = self.criar_linha_resultado(
            self.frame_resultados, 2, "Tangente:"
        )
    # Cria um par (label descritiva + label de resultado) numa linha do grid.
    # Método auxiliar para repetir este mesmo bloco 3 vezes.
    def criar_linha_resultado(self, frame, linha, texto):
        tk.Label(
            ...
            bd=0,
            bg="#f0f0f0"
        ).grid(row=linha, column=0, padx=10, pady=5, sticky="w")
